...
namespace EADProj.DLL
{
    public class FCommentDAO
    {
        ...
        public void Insert(FComment fcomment)
        {
            ...
            // sqlCmd.Parameters.AddWithValue("@paraImageURL", fcomment.img);
        ...
        public FComment GetCommentByID(string comment_id)
        {
            ...
            else
            {
                ...
                FComment comment1 = new FComment(row["user_id"].ToString(), row["comment"].ToString(), Convert.ToInt16(row["votes"]), Convert.ToInt16(row["post_id"]), row["imageURL"].ToString());
                return comment1;
            }
        }
